...
docker on dockerにしないためにwebsh_serverをホストPC上で動作させるようにしたのと
デプロイをシンボリックリンクで切り替える構成にした関係でnginxをコンテナ上で起動してhtmlをマウントできなかった。
しかたなくローカルと構成が違うけれど、nginxもホストPC上で動作させるようにした。
本番環境ではシェル芸botコンテナのみDockerで動作させている。
